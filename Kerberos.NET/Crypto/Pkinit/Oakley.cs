using System;

namespace Kerberos.NET.Crypto
{
    public static class Oakley
    {
        private static byte[] Reverse(ReadOnlyMemory<byte> data)
        {
            var copy = new byte[data.Length];

            data.CopyTo(copy);

            Array.Reverse(copy);

            return copy;
        }

        public static class Group14
        {
            /*
             * https://tools.ietf.org/html/rfc3526#section-3
             * Id: 14
             * Prime: 2^2048 - 2^1984 - 1 + 2^64 * { [2^1918 pi] + 124476 }
             * Generator: 2
             */

            public static ReadOnlyMemory<byte> Prime = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> PrimeLittleEndian = Reverse(Prime);

            public static ReadOnlyMemory<byte> Generator = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> GeneratorLittleEndian = Reverse(Generator);

            public static ReadOnlyMemory<byte> Factor = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> FactorLittleEndian = Reverse(Factor);
        }

        public static class Group2
        {
            public static ReadOnlyMemory<byte> Prime = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> PrimeLittleEndian = Reverse(Prime);

            public static ReadOnlyMemory<byte> Generator = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> GeneratorLittleEndian = Reverse(Generator);

            public static ReadOnlyMemory<byte> Factor = new byte[]
            {
                
            };

            public static ReadOnlyMemory<byte> FactorLittleEndian = Reverse(Factor);
        }
    }
}

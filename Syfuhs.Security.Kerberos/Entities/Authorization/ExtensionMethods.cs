...
namespace Syfuhs.Security.Kerberos.Entities.Authorization
{
    ...
    {
        public static string GetFriendlyName(this SecurityIdentifier sid)
        {
            var sddl = sid.Value.Replace(sid.AccountDomainSid.Value, "S-1-5-21-<domain>");

            string name;

            if (WellKnownSids.TryGetValue(sddl, out name))
            {
                return name;
            }
        ...
        private static readonly Dictionary<string, string> WellKnownSids = new Dictionary<string, string> {
            ...
            { "S-1-5-32-554", "Alias Prew2kcompacc" },
